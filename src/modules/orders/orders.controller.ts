...
export class OrdersController {
    ...
    async rateOrder(req: AuthRequest, res: Response) {
        try {
            const { rating, merchantRating, courierRating, comment } = req.body;
            const effectiveMerchantRating = merchantRating ?? rating;
            if (!effectiveMerchantRating) {
                return sendResponse(res, 400, false, 'merchantRating (or rating) is required');
            }

            const review = await ordersService.rateOrder(req.user.id, req.params.orderId as string, {
                rating,
                merchantRating,
                courierRating,
                comment
