...
export class ConsumerController {
    ...
    async updateProfile(req: AuthRequest, res: Response) {
        try {
            ...
            // Auto-update full_name if first_name/last_name are modified
            if (body.first_name !== undefined || body.last_name !== undefined) {
                const existingUser = await consumerService.getProfile(req.user.id);
                const currentFirstName = body.first_name !== undefined ? body.first_name : (existingUser?.first_name || '');
                const currentLastName = body.last_name !== undefined ? body.last_name : (existingUser?.last_name || '');
                body.full_name = `${currentFirstName} ${currentLastName}`.trim();
            }

            // Strictly filter only the valid database columns for Supabase update
            const updatePayload: any = {};
            const allowedFields = ['first_name', 'last_name', 'full_name', 'avatar_url', 'date_of_birth', 'email'];
            ...
            // Convert empty string date_of_birth to null (PostgreSQL DATE rejects '')
            if (updatePayload.date_of_birth !== undefined && !updatePayload.date_of_birth) {
                updatePayload.date_of_birth = null;
            }

            const profile = await consumerService.updateProfile(req.user.id, updatePayload);
