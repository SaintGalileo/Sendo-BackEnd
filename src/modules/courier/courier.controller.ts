...
export class CourierController {
    ...
    async getOrderById(req: AuthRequest, res: Response) {
        try {
            const order = await courierService.getOrderByIdForCourier(req.user.id, req.params.orderId as string);
    ...
    async getDeliveries(req: AuthRequest, res: Response) {
        try {
            const status = req.query.status as 'ongoing' | 'completed' | 'cancelled' | undefined;
            const allowedStatuses = ['ongoing', 'completed', 'cancelled'] as const;

            if (status && !allowedStatuses.includes(status)) {
                return sendResponse(res, 400, false, 'Invalid status. Use ongoing, completed, or cancelled');
            }

            const deliveries = status
                ? await courierService.getDeliveries(req.user.id, status)
                : await courierService.getDeliveries(req.user.id);
            return sendResponse(
                res,
                200,
                true,
                status ? `${status} deliveries fetched` : 'Courier deliveries fetched',
                deliveries
            );
        } catch (error: any) {
            return sendResponse(res, 500, false, error.message);
